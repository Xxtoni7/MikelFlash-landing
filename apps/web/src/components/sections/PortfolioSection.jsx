...
function getCardScore(rect, visibleHeight, viewportCenter) {
    const cardCenter = rect.top + rect.height / 2;
    const visibility = visibleHeight / Math.max(rect.height, 1);

    return Math.abs(cardCenter - viewportCenter) - visibility * 80;
}

function getClosestVisibleCardId(cardRefs) {
    ...
    for (const [id, node] of cardRefs.current.entries()) {
        ...
        const score = getCardScore(rect, visibleHeight, viewportCenter);
...
function PortfolioSection({ portfolioItems, scrollToContact }) {
    ...
    const activeMobileProjectIdRef = useRef(null);
    ...
    useEffect(() => {
        activeMobileProjectIdRef.current = activeMobileProjectId;
    ...
    useEffect(() => {
        ...
        const updateActiveProjectId = (nextId) => {
            if (activeMobileProjectIdRef.current === nextId) return;

            activeMobileProjectIdRef.current = nextId;
            setActiveMobileProjectId(nextId);
        };
        ...
        const updateActiveCard = () => {
            ...
            frameId = requestAnimationFrame(() => {
                ...
                updateActiveProjectId(getClosestVisibleCardId(cardRefs));
            });
        };
                ...
                {selectedProject && (
                    ...
                    >
                        <motion.div
                            ...
                            className="relative h-auto max-h-[calc(100dvh-1rem)] w-full max-w-6xl overflow-hidden rounded-[1.5rem] border border-primary/20 bg-card shadow-2xl shadow-black/50 sm:max-h-[82vh] sm:rounded-[2rem] lg:h-auto lg:max-h-[92vh]"
                        >
                            <button
                                onClick={closeProject}
                            ...
                            <div className="grid max-h-[calc(100dvh-1rem)] grid-rows-[250px_minmax(0,1fr)] sm:grid-rows-[300px_minmax(0,1fr)] lg:h-full lg:max-h-none lg:grid-rows-none lg:grid-cols-[1.15fr_0.85fr]">
                                <div className="relative h-[250px] overflow-hidden bg-black sm:h-[300px] lg:h-[620px] lg:min-h-[620px]">
                                ...
                                <div className="relative flex min-h-0 flex-col p-6 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:p-7 md:p-10">
                                    ...
                                    <div className="relative z-10 flex h-full min-h-0 flex-col">
                                        <div className="min-h-0 flex-1 overflow-y-auto pr-1 md:overflow-visible md:pr-0">
                                            <h3 className="text-2xl font-bold text-foreground leading-tight md:mt-6 md:text-4xl">
